...
public class GetLosersQueryHandlerTests : UnitTestsBase
{
    [Test]
    public async Task GetsLosersCryptocurrencies()
    {
        // Arrange
        ...
        Client.Setup(x => x.GetLosers(CancellationToken.None)).ReturnsAsync(cryptocurrencies);


        // Act
        var result = (await SendCommand(new GetLosersQuery())).ToList();
