...
public class GetGainersQueryHandlerTests : UnitTestsBase
{
    [Test]
    public async Task GetsGainersCryptocurrencies()
    {
        // Arrange
        ...
        Client.Setup(x => x.GetGainers(CancellationToken.None)).ReturnsAsync(cryptocurrencies);


        // Act
        var result = (await SendCommand(new GetGainersQuery())).ToList();
